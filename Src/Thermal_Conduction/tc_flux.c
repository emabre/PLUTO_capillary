// version modified by Ema, to fix the problem that with the use of pvte_law g_gamma is not defined
 ...
 *********************************************************************** */
{
  ...
  #elif EOS==PVTE_LAW
    /*[Ema] This is provisory, I take the g_gamma=5/3 (g_gamma is the adiabatic index), maybe the g_gamma should be more accurate?
    At page 69 of the doc they say that an accurate gamma is not very important, also here it seems that it is
    only needed to compute the time step. Since higher gamma means higher sound speed, I guess I will be safe if I take
    the maximum value possible, which is, I guess 5/3 (in an ideal gas: gamma=1+2/f, where f are the degrees of freedom).
    You could use Gamma1() (defined in pvtw_law.c) to improve this */
      double Fc, Fcmag, Fsat, g1 = 5/3 - 1.0;
  #endif
